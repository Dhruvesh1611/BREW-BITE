...
export default function CartPage() {
  ...
  if (cart.length === 0) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-[#FDFCF7]">
        <div className="bg-white rounded-[2.5rem] p-12 shadow-xl border border-[#EBE4D5] text-center max-w-md">
          <div className="h-24 w-24 bg-[#FDFCF7] rounded-full flex items-center justify-center mx-auto mb-6">
            <ShoppingCart className="h-12 w-12 text-[#8C8775]" />
          </div>
          <h2 className="text-3xl font-bold text-[#3E2B21] mb-4">Cart is Empty</h2>
          <p className="text-[#8C8775] mb-8">Add some products to get started</p>
          <button
            onClick={() => window.location.href = '/pos/terminal'}
            className="px-8 py-4 bg-[#3E2B21] text-white rounded-[2rem] font-bold hover:bg-[#2C1810] transition-all shadow-lg hover:shadow-xl"
          >
            Browse Products
          </button>
        </div>
          ...
          <div className="flex-1 lg:overflow-y-auto lg:pr-2">
            <div className="space-y-4">
              {cart.map((item) => (
                <div
                  key={item.cartItemId}
                  className="bg-white rounded-[2rem] p-6 shadow-md border border-[#EBE4D5] hover:shadow-lg transition-all"
                >
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <h3 className="text-xl font-bold text-[#3E2B21] mb-1">{item.name}</h3>
                            <p className="text-sm text-[#8C8775]">
                        ...
                        <div className="mt-3">
                          <label className="block text-[11px] font-bold uppercase tracking-wider text-[#8C8775] mb-1">
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 focus:border-[#3E2B21] focus:outline-none bg-[#FDFCF7]/50 font-medium text-[#3E2B21]"
                          />
                        </div>
                      </div>

                      {/* Quantity & Price */}
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-[#FDFCF7]">
                        <div className="flex items-center gap-3 bg-[#FDFCF7] rounded-xl p-1.5 border border-[#EBE4D5]">
                          <button
                            onClick={() => decreaseQuantity(item.cartItemId)}
                            className="h-9 w-9 hover:bg-white rounded-lg flex items-center justify-center transition-colors text-[#8C8775]"
                          >
                            <Minus className="h-4 w-4" />
                          </button>
                          <span className="w-8 text-center font-bold text-[#3E2B21] text-md">{item.quantity}</span>
                          <button
          ...
          <div className="w-full lg:w-96 space-y-4 sm:space-y-6">
            ...
            <div className="bg-white rounded-[2rem] p-6 shadow-md border border-[#EBE4D5]">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-[#3E2B21]">Customer</h3>
                <button
                  onClick={() => setIsCustomerModalOpen(true)}
                  className="p-2 hover:bg-[#FDFCF7] rounded-xl transition-colors"
                >
                  <Edit2 className="h-4 w-4 text-[#8C8775]" />
                </button>
              </div>
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${customer ? 'bg-[#EBE4D5] text-[#3E2B21]' : 'bg-gray-100 text-gray-400'}`}>
                    <User className="h-5 w-5" />
                  </div>
                  <div>
                    <p className="font-semibold text-[#3E2B21]">{customer?.name || 'Walk-in Customer'}</p>
                    {customer?.mobile && (
                      <p className="text-sm text-[#8C8775]">{customer.mobile}</p>
                    )}
                  </div>
                </div>
                {customer?.email && (
                  <p className="text-sm text-[#8C8775] pl-13">{customer.email}</p>
                )}
              </div>
            </div>

            {/* Coupons & Promo Codes */}
            <div className="bg-white rounded-[2rem] p-6 shadow-md border border-[#EBE4D5] space-y-3">
              <h3 className="font-bold text-[#3E2B21] flex items-center gap-2">
                <Tag className="h-5 w-5 text-[#3E2B21]" />
                Promo / Coupon
              </h3>
              ...
              {!coupon ? (
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2 border rounded-xl focus:border-[#3E2B21] focus:outline-none uppercase font-bold text-sm"
                  />
                  <button
                    onClick={handleApplyCoupon}
                    disabled={validatingCoupon || !couponInput}
                    className="px-4 py-2 bg-[#3E2B21] text-white rounded-xl font-bold hover:bg-[#2C1810] text-sm disabled:opacity-50"
                  >
                    {validatingCoupon ? '...' : 'Apply'}
                  </button>
                </div>
              ) : (
                <div className="p-3 bg-[#EBE4D5] rounded-xl border border-[#C4A882]/30 flex justify-between items-center">
                  <div>
                    <span className="font-black text-[#3E2B21] text-sm">{coupon.code}</span>
                    <span className="text-xs text-[#8C8775] ml-2">({coupon.type === 'PERCENTAGE' ? `${coupon.discount}%` : `₹${coupon.discount}`} off)</span>
                  </div>
            ...
            {/* Order Summary */}
            <div className="bg-white rounded-[2rem] p-6 shadow-md border border-[#EBE4D5] space-y-4">
              <h3 className="font-bold text-[#3E2B21] text-lg mb-4">Order Summary</h3>

              <div className="space-y-3">
                <div className="flex justify-between text-[#8C8775]">
                ...
                <div className="border-t border-[#EBE4D5] pt-3">
            ...
            {/* Table Info */}
            {selectedTable && (
              <div className="bg-[#EBE4D5] rounded-[2rem] p-4 border border-[#C4A882]/30 text-center">
                <p className="text-xs text-[#8C8775] uppercase tracking-wider font-bold">Selected Table</p>
                <p className="font-black text-[#3E2B21] text-xl mt-1">{selectedTable.name}</p>
              </div>
            )}
          </div>
        </div>
